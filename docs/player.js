let media="https://raw.githubusercontent.com/PerseusInLSC/Perseus-Study-Music/refs/heads/main/media/"
...
request.onload=function(){
    ...
    // Randomly select a track on startup instead of the last one
    playNum = Math.floor(Math.random() * jsonData.length); // Random index
...
Player.prototype = {
  ...
  play: function(index) {
    ...
    document.title=data.title + " - Perseus' Music Player";//显示浏览器TAB栏内容
  ...
  skip: function(direction) {
    ...
    let index = self.index; // Start with the current index
    if (direction === 'next') {
      index = self.index + 1; // Increment the index for the next track
      if (index >= self.playlist.length) {
        index = 0; // Loop back to the first track if at the end
      }
    } else {
      index = self.index - 1; // For previous track
      if (index < 0) {
        index = self.playlist.length - 1; // Loop back to the last track if at the beginning
      }
    }
